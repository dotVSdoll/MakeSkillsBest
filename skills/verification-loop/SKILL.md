...
meta.verificationMode == "security-tool"    → Template D: Security Tool 验证链
```
...
### Template D: Security Tool 验证链

**适用：** security-tool 类型项目（security scanner / vulnerability detector / SAST）

| 级别 | 验证内容 | 方法 |
|---|---|---|
| L1-install | 可安装 | pip install / npm install / go install |
| L2-help | 能输出帮助 | `tool --help` 或 `tool scan --help` |
| L3-safe-fixture | 扫描已知安全的 fixture | `tool scan ./fixtures/safe/` → exit 0 + 无 finding |
| L4-bad-fixture | 扫描已知有漏洞的 fixture | `tool scan ./fixtures/vuln/` → exit > 0 + 包含 finding |
| L5-report | 报告结构正确 | 输出包含 severity / category / recommendation / location |

**fixture 要求：**
- L3 safe fixture: 不含任何已知漏洞模式的代码
- L4 bad fixture: 至少包含 1 个 SQL 注入 + 1 个硬编码密钥
- 如果项目自带 test fixtures → 优先使用
